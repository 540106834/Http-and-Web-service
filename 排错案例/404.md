你这条 `curl -v` 的输出其实已经把“案发现场”交代得非常清楚了，我们来像剥洋葱一样一层层拆 🧅。

---

## 一眼看结论

**你的请求发到了对的 Service，但打到了“没有被任何路由处理的路径 `/`”，所以后端返回了 404。**

> DNS ✔
> Service 连通 ✔
> Pod 能收到请求 ✔
> HTTP 路由未匹配 ❌ → 返回 `{ "status": 404, "path": "/" }`

---

## 逐行解剖你这次请求

### 1. DNS 解析 ✅

```bash
* Host bd-report-api.bigdata.svc.cluster.local:80 was resolved.
* IPv4: 192.168.220.117
```

说明：

* K8s 内部 DNS 生效
* 域名解析到 **ClusterIP：192.168.220.117**
* 这是你这个 Service 的虚拟 IP

---

### 2. TCP 连接 ✅

```bash
*   Trying 192.168.220.117:80...
* Connected to ...
```

说明：

* kube-proxy 转发表工作正常
* Service → Endpoint → Pod 链路没问题

---

### 3. 请求内容

```http
> POST / HTTP/1.1
> Host: bd-report-api.bigdata.svc.cluster.local
```

关键点：

* 你请求的 **路径是：`/`**
* 方法是 `POST`

---

### 4. 服务返回

```http
< HTTP/1.1 404 Not Found
< Content-Type: application/json
...
{"timestamp":"2025-12-10 17:16:06","status":404,"error":"Not Found","path":"/"}
```

这个返回格式非常“Spring Boot 风格”，典型的：

* 请求已经进入到 Web 应用
* 但 **没有任何 Controller / Router 能处理 `/` + `POST`**

---

## ⚖ 所以问题根因是

> ✅ Service 和网络没问题
> ❌ 访问的 **路径不对**

你的后端服务**大概率根本没有定义 `/` 这个接口**。

---

## 正确排查姿势

### ✅ 1. 查看 Service 暴露了哪些端口

```bash
kubectl get svc -n bigdata bd-report-api -o yaml
```

重点看：

```yaml
spec:
  ports:
    - port: 80
      targetPort: 8080   # 容器内部端口
```

---

### ✅ 2. 找 Pod，查看真正的接口

```bash
kubectl get ep -n bigdata bd-report-api
```

然后进入任一 Pod：

```bash
kubectl exec -it <pod> -n bigdata -- sh
```

---

### ✅ 3. 查询 Spring Boot 接口列表

最简单直接的几种方式：

#### 如果有 actuator：

```bash
curl http://localhost:8080/actuator/mappings
```

---

#### 或 grep 日志：

```bash
grep -R "RequestMapping" logs/
```

---

#### 或扫一下接口试试

```bash
curl http://bd-report-api.bigdata.svc.cluster.local/health
curl http://bd-report-api.bigdata.svc.cluster.local/api
curl http://bd-report-api.bigdata.svc.cluster.local/report
```

---

## ✅ 正确示例

假设你的接口是：

```java
@PostMapping("/report")
```

那么正确调用方式是：

```bash
curl -X POST \
  http://bd-report-api.bigdata.svc.cluster.local/report \
  -H "Content-Type: application/json" \
  -d '{"foo":"bar"}'
```

---

## 为什么不是 503 / 502？

很多人以为 service 配错会 404，其实完全相反：

| 错误码                              | 真正含义           |
| -------------------------------- | -------------- |
| **404**                          | 服务收到请求，但路由没匹配  |
| **503 / 502**                    | 请求根本没能打到后端 Pod |
| **Connection refused / timeout** | 网络或端口层面失败      |
| **NXDOMAIN**                     | DNS 根本没解析到     |

你这里是最健康的一种失败模式：

✅ 请求完全走通
❌ 只是走错门

---

## 最小诊断总结 🧭

```text
curl -> Cluster DNS -> Service ClusterIP -> Endpoint -> Pod -> Spring MVC

            ✅               ✅              ✅              ✅
                                                   |
                                                   ❌
                                           没有匹配 "/"
```

---

## 一句话结论

> Service 没错，网络没错，Pod 没问题，
> **错的是：你 POST 了一个后端没有实现的 `/` 路径。**

---

## 如果你愿意

你贴一下：

```bash
kubectl get svc -n bigdata bd-report-api -o yaml
```

和

```bash
kubectl logs -n bigdata <任意一个bd-report-api-pod>
```

我可以帮你**把真实接口路径直接扒出来**，给你一条“必 200 的 curl”。 🧙‍♂️
